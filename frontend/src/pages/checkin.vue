        ...
        <v-main style="background-color: #faf7f2">
            ...
            <MealDialog
                ...
                @update:consumption-level="
                    mealEditData.consumptionLevel = $event
                "
            ...
            <GrowthDialog
                ...
                @update:head-circumference="
                    growthEditData.headCircumference = $event
                "
...
<script setup lang="ts">
    ...
    const { mealTimeOptions, mealCategoryOptions, consumptionOptions } =
        useMealOptions()
    ...
    // Helper function to capitalize account names
    // const capitalizeAccountName = (accountName: string): string => {
    //   if (!accountName) return 'Unknown'
    //   return accountName.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')
    // }
    const getAccount = (
        ...
        return accountMap[accountId || 0] || 'Unknown'
        472
    }
    ...
    const convertMealsToTimeline = (meals: any[]) => {
        return meals.map((meal) => ({
            ...
            carerName: getAccount(meal.account_id), // Use helper function
            data: {
    ...
    const convertPoopToTimeline = (poops: any[]) => {
        return poops.map((poop) => ({
            ...
            carerName: getAccount(poop.account_id), // Use helper function
            data: {
    ...
    const convertSleepToTimeline = (sleepEntries: any[]) => {
        return sleepEntries.map((sleep) => ({
            ...
            carerId: 2,
            carerName: getAccount(sleep.account_id), // Use helper function
            data: {
    ...
    const convertGrowthToTimeline = (growthEntries: any[]) => {
        return growthEntries.map((growth) => ({
            ...
            carerId: 1,
            carerName: getAccount(growth.account_id), // Use helper function
            data: {
    ...
    const convertHealthToTimeline = (healthEntries: any[]) => {
        return healthEntries.map((health) => ({
            ...
            carerId: 3,
            carerName: getAccount(health.account_id), // Use helper function
            data: {
    ...
    const formatMealStatus = (meal: any) => {
        ...
        if (meal.time_category) {
            const timeCategory =
                meal.time_category.charAt(0).toUpperCase() +
                meal.time_category.slice(1)
        ...
        if (
            ...
            timeCategoryNames[meal.meal_time_category]
        ) {
    ...
    const formatMealDetails = (meal: any) => {
        ...
        if (
            meal.consumption_level !== undefined &&
            meal.consumption_level !== null
        ) {
            parts.push(`consumed ${meal.consumption_level}%`)
        }

        const result =
            parts.length > 0 ? parts.join(', ') : 'No details available'
    ...
    const formatSleepStatus = (sleep: any) => {
        const bedTime = sleep.start_time
        ...
        const awakeTime = sleep.end_time
            ? new Date(sleep.end_time).toLocaleTimeString('en-US', {
                  hour: '2-digit',
                  minute: '2-digit',
                  hour12: true,
              })
    ...
    const formatGrowthDetails = (growth: any) => {
        ...
        if (growth.head_circumference)
            parts.push(`Head: ${growth.head_circumference}cm`)
 ...
 */
    ...
    const checkinsByChild = computed(() => {
        if (!childrenStore.currentChild) return []
        return realCheckins.value.filter(
    ...
    const confirmDelete = async () => {
        ...
        try {
            if (checkin.type === 'meal' && checkin.rawData) {
                await fetch(
                    `http://127.0.0.1:8000/meal/${checkin.rawData.id}`,
                    { method: 'DELETE' },
                )
            } else if (checkin.type === 'poop' && checkin.rawData) {
                await fetch(
                    `http://127.0.0.1:8000/poop/${checkin.rawData.id}`,
                    { method: 'DELETE' },
                )
            } else if (checkin.type === 'sleep' && checkin.rawData) {
                await fetch(
                    `http://127.0.0.1:8000/sleep/${checkin.rawData.id}`,
                    { method: 'DELETE' },
                )
            } else if (checkin.type === 'growth' && checkin.rawData) {
                await fetch(
                    `http://127.0.0.1:8000/growth/${checkin.rawData.id}`,
                    { method: 'DELETE' },
                )
            } else if (checkin.type === 'health' && checkin.rawData) {
                await fetch(
                    `http://127.0.0.1:8000/symptom/${checkin.rawData.id}`,
                    { method: 'DELETE' },
                )
            }
    ...
    const openMealEditDialog = (checkin: any) => {
        ...
        mealEditData.value = {
            ...
            consumptionLevel: mapConsumptionLevelToValue(
    ...
    const handleMealEditSave = async () => {
        ...
        try {
            ...
            const originalMeal = rawMealsData.value.find(
                (meal) => meal.id === mealId,
            )
            if (!originalMeal) {
                ...
                console.log(
                    '📋 Available meals:',
                    rawMealsData.value.map((m) => ({ id: m.id, note: m.note })),
                )
            ...
            const mealCategoryId = mealCategoryOptions.value.find(
                (opt) => opt.value === mealEditData.value.mealCategory,
            )?.id
            console.log('🔍 Mapped meal category ID:', mealCategoryId)

            if (!mealCategoryId) {
                console.error(
                    '❌ Could not find meal category ID for:',
                    mealEditData.value.mealCategory,
                )
            ...
            const updatePayload = {
                ...
                consumption_level: parseFloat(
                    mealEditData.value.consumptionLevel,
                ),
                meal_category: mealCategoryId, // Only this can be changed
                others:
                    mealEditData.value.mealCategory === 'others'
                        ? mealEditData.value.customMeal
                        : originalMeal.others || null,
            ...
            console.log(
                `🔒 PRESERVING original check_in: ${originalMeal.check_in}`,
            )
            console.log(
                `🔒 PRESERVING original meal_time_category: ${originalMeal.meal_time_category}`,
            )
            console.log(`📝 UPDATE payload:`, updatePayload)

            // Call the PUT update API (this should UPDATE existing record, not create new)
            const response = await fetch(
                `http://127.0.0.1:8000/meal/${mealId}`,
                {
                    method: 'PUT',
                    headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    `Failed to update meal: ${response.status} ${response.statusText} - ${errorText}`,
                )
            }
            ...
            console.log(
                '🔍 Updated meal meal_time_category:',
                updatedMeal.meal_time_category,
            )
            ...
            console.log(
                '✅ Edit completed - should see only updated record, no duplicates',
            )
        } catch (error) {
    ...
    const handlePoopEditSave = async (poopData) => {
        ...
        try {
            ...
            const originalPoop = rawPoopData.value.find(
                (poop) => poop.id === poopId,
            )
            if (!originalPoop) {
            ...
            const colorId = colorOptions.value.find(
                (opt) => opt.value === poopData.color,
            )?.id
            const textureId = textureOptions.value.find(
                (opt) => opt.value === poopData.texture,
            )?.id
            ...
            // Call API
            const response = await fetch(
                `http://127.0.0.1:8000/poop/${poopId}`,
                {
                    method: 'PUT',
    ...
    const handleSleepEditSave = async (sleepData) => {
        ...
        try {
            ...
            const originalSleep = rawSleepData.value.find(
            ...
            const response = await fetch(
                `http://127.0.0.1:8000/sleep/${sleepId}`,
                {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(updatePayload),
    ...
    const handleGrowthEditSave = async (growthData) => {
        ...
        try {
            ...
            const originalGrowth = rawGrowthData.value.find(
                (growth) => growth.id === growthId,
            )
            if (!originalGrowth) {
            ...
            const response = await fetch(
                `http://127.0.0.1:8000/growth/${growthId}`,
                {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(updatePayload),
    ...
    const openHealthEditDialog = (checkin: any) => {
        ...
        if (health.symptom) {
            const symptomList = health.symptom
                .split(',')
                .map((s) => s.trim())
                .filter((s) => s)
            ...
            for (const dbSymptom of symptomList) {
                ...
                const matchingOption = symptomOptions.value.find(
                    (option) =>
                ...
                if (matchingOption) {
                    console.log(
                        `✅ Mapped "${dbSymptom}" to "${matchingOption.value}"`,
                    )
                    mappedSymptoms.push(matchingOption.value)
                } else {
                    // If no match found, treat as custom symptom
                    console.log(
                        `❓ No match for "${dbSymptom}", treating as custom`,
                    )
    ...
    const handleHealthEditSave = async (healthData) => {
        ...
        try {
            ...
            const originalHealth = rawHealthData.value.find(
                (health) => health.id === healthId,
            )
            if (!originalHealth) {
            ...
            if (healthData.symptoms && healthData.symptoms.length > 0) {
                ...
                if (
                    healthData.symptoms.includes('other') &&
                    healthData.otherSymptom
                ) {
            ...
            const response = await fetch(
                `http://127.0.0.1:8000/symptom/${healthId}`,
                {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(updatePayload),
        ...
        try {
            ...
            const promises = [
                ...
                fetch(`http://127.0.0.1:8000/meal/child/${childId}?days=60`)
                    ...
                    .then((allMeals) => {
                        ...
                        if (allMeals.length > 0) {
                            console.log('🔍 Sample meal data:', allMeals[0])
                        }
                ...
                fetch(`http://127.0.0.1:8000/sleep/child/${childId}?days=60`)
                    .then((res) => res.json())
                    .then((allSleep) => {
                        console.log('🔍 Sample slep data:', allSleep[0])
            ...
            console.log(
                `✅ All data loaded - let filteredCheckins do the filtering!`,
            )
        } catch (error) {
